...
class YoloGuidanceEngine(IGuidanceEngine):
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        self.phase4_done = False  # Phase 4 idempotency flag
    ...
    def maybe_finalize(self, session_id: str) -> Dict[str, Any]:
        ...
        if (most_frequent_class == FINAL_CAPTURE_CLASS and 
            ...
            # Select best class-5 frame, preferring the last 2s window
            selected_frame = None
            try:
                now_ts = time.time()
                # Candidates: class-5 within aggregation window
                class5_window = [
                    f for f in self.frame_buffer
                    if getattr(f.get("guidance"), "dir", None) == GuidanceDirection.PERFECT
                    and (now_ts - f.get("server_ts", now_ts)) <= self.aggregation_window
                ]
                if class5_window:
                    selected_frame = max(class5_window, key=lambda f: f.get("score", 0.0))
                else:
                    # Fallback: any class-5 in buffer (most recent)
                    class5_all = [
                        f for f in self.frame_buffer
                        if getattr(f.get("guidance"), "dir", None) == GuidanceDirection.PERFECT
                    ]
                    if class5_all:
                        selected_frame = class5_all[-1]
                    else:
                        # Last resort: previous global best
                        selected_frame = self.best_frame
            except Exception as e:
                log_debug("final_pick", f"Selection error, falling back to best_frame: {str(e)}", session_id=session_id)
                selected_frame = self.best_frame

            # Get best frame from selection
            if selected_frame is not None:
                ...
                try:
                    # Always prefer the original JPEG bytes from the selected frame
                    jpeg_bytes = selected_frame.get("data")
                    if isinstance(jpeg_bytes, (bytes, bytearray)) and len(jpeg_bytes) > 0:
                        with open(save_path, 'wb') as f:
                            f.write(jpeg_bytes)
                    else:
                        # Encode from BGR array
                        success, encoded_img = cv2.imencode('.jpg', selected_frame.get("image"), 
                    ...
                    log_info("final_selected", 
                        f"Best frame saved: path={save_path}, score={selected_frame.get('score', 0.0):.3f}",
                            session_id=session_id)
                    ...
                    return {
                        "triggered": True,
                    "best_frame_jpeg": jpeg_bytes if isinstance(jpeg_bytes, (bytes, bytearray)) else None,
                        "save_path": relative_path,
                    "confidence": selected_frame.get('score', 0.0)
                    }
    ...
    def reset_session_state(self):
        ...
        self.best_frame_score = 0.0
        self.last_guidance_time = 0.0
        # Reset Phase 4 idempotency flag for new sessions
        if hasattr(self, 'phase4_done'):
            self.phase4_done = False
